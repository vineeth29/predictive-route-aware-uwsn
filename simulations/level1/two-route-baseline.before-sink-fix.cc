...
static void
TraceSinkSocketRx(Ptr<Socket> socket)
{
    if (socket == nullptr)
    {
        return;
    ...
    bool error)
{
    if (g_eventFile != nullptr)
    {
        (*g_eventFile)
            ...
            << nodeId
            << ","
            << (packet != nullptr ? packet->GetUid() : 0)
            << ","
            << rxPower
            << ","
            << interference
    ...
    double noise)
{
    if (g_eventFile != nullptr)
    {
        ...
        if (packet != nullptr && packet->PeekHeader(ash))
        {
            error = ash.GetErrorFlag();
...
main(int argc, char* argv[])
{
    ...
    // Built-in Aqua-Sim channel trace driver
    AquaSimTraceReader tReader;
    tReader.SetChannel(aquaHelper.GetChannel());
    tReader.ReadFile("channelTrace.txt");
         ...
         ++i)
    {
        ...
        // ----------------------------------------------------
        // REAL SIGNAL-CACHE SINR trace
        // ----------------------------------------------------

        Ptr<AquaSimSignalCache> signalCache =
            phy->GetSignalCache();

        signalCache->TraceConnectWithoutContext(
            "SINR",
            MakeBoundCallback(
                &TraceSinr,
                i));
    ...
    sinkSocket->SetRecvCallback(
        MakeCallback(&TraceSinkSocketRx));
    ...
    std::string sinkPacketReceivedPath =
        "/NodeList/" +
        std::to_string(nodes - 1) +
        "/DeviceList/*/$ns3::NetDevice/Routing/PacketReceived";

    Config::ConnectWithoutContext(
        sinkPacketReceivedPath,
